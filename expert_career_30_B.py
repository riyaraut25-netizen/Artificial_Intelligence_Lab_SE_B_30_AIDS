...
class CareerExpertSystem(KnowledgeEngine):
    ...
    def aids(self):
        print("Suggested Career Path : AIDS Branch ")
    @Rule(StudentFacts(likes='Programming'), StudentFacts(likes='Artificial Intelligence'))
    def aiml(self):
        print("Suggested Career Path : AIML Branch ")

def main():
    engine = CareerExpertSystem()
    engine.reset()
    print("Welcome to the Career Path Expert System!")
    interests = input("Enter your interests separated by commas (e.g., Maths, Physics, Programming, Statistics, Biology, Chemistry, Circuits, Artificial Intelligence): ").split(',')
